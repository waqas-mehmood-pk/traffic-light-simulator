...
use TrafficLight\YellowLightOffState;
use TrafficLight\TrafficLight;

class YellowLightOffTest extends \PHPUnit_Framework_TestCase {
	
	/** @test */
	public function timer_can_not_be_more_than_time_should_be_on () {
		$state = new YellowLightOffState( new TrafficLight() );
		$state->onOff();
	...
	public function class_should_be_implement_the_interface () {
		$light = new YellowLightOffState(new TrafficLight());
	...
	public function state_info_should_be_return_green_light () {
		$state = new YellowLightOffState( new TrafficLight() );
		$this->assertEquals( 'Yellow Light Off', $state->stateInfo() );
	}
